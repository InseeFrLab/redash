...
class Datalab(BaseSQLQueryRunner):
    ...
    def _get_tables(self, schema):
        ...
        columns_query = "describe table %s.%s"
        logger.info("gettables")
        logger.info(self._run_query_internal(schemas_query))
        for schema_name in filter(lambda a: len(a) > 0, map(lambda a: str(a['namespace']), self._run_query_internal(schemas_query))):
            ...
            for table_name in filter(lambda a: len(a) > 0, map(lambda a: str(a['tableName']), self._run_query_internal(tables_query % schema_name))):
                ...
                try:
                    logger.info(self._run_query_internal(columns_query % (schema_name, table_name)))
                    columns=[]
                    for a in self._run_query_internal(columns_query % (schema_name, table_name)):
                        nomColonne = str(a['col_name'])
                        if "#" not in nomColonne: 
                            columns.append(nomColonne)
                        else:
                            break
                    ...
                    logger.info("mise a jour values")
                    logger.info(schema.values())
                except:
                    log.error("erreur sur les colonnes de "+table_name)
                    continue
        logger.info(schema.values())
